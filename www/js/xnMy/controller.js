...
>>>>>>> 8b0c5b038174469ec9a596b16a6d9df57b4801f7
      ...
      vm.changeUser = function(str, cb) {
        if (str == 'username') {
          var obj = {
            title: '修改昵称',
            templateUrl:
              '<input type="text" ng-model="data.username" placeholder="请输入昵称...">',
            btn: '确定'
          }
          var query = {
            type: str
...
<<<<<<< HEAD
      ...
      $scope.getCoke = function(phone) {
        if (
          /(^13[0-9]{9}$)|(^15[0-9]{9}$)|(^17[0-9]{9}$)|(^18[012356789][0-9]{8}$)/.test(
            phone
          )
        ) {
          vm.clearIntervalOut(true)
        ...
        AccountService.xnGetChangeCode(phone).then(
          function(res) {
            if (res.status == 200) {
              vm.clearIntervalOut(true)
              //  Storage.set('userInfo', res.data);
              ...
              if (res.status == 404 && res.msg == 'Verify Code Is Wrong') {
                ...
                res.status == 402 &&
                res.msg == 'Phone Number Have Already Register'
              ) {
                ionicToast.show(
                  '获取验证码错误，该手机号已注册',
                  'middle',
      ...
      vm.submitChange = function(obj) {
        console.log('改变', obj)
        if (!angular.isString(obj)) {
          FirstService.changeUserInfo(obj).then(
            function(res) {
              if (res.status == 200) {
                ionicToast.alert('修改成功')
                Storage.set('userInfo', res.data)
...
>>>>>>> 8b0c5b038174469ec9a596b16a6d9df57b4801f7
          ...
          //修改手机号
          AccountService.xnGetChangePhone(obj).then(
            function(res) {
              if (res.status == 200) {
                ionicToast.alert('修改成功,请重新登录')
                Storage.remove('userInfo')
...
<<<<<<< HEAD
  .controller('order_details_ctrl', [
    ...
    function(
      ...
      vm.init = function() {
        FirstService.getOrderDetails($stateParams.id)
          .then(
            function(res) {
              $log.debug('获取订单详情33', res)
              if (res.status == 200) {
                vm.order = res.data
                vm.dele.isLoading = true
                if (vm.order) {
                  if (vm.order.order_status == '1') {
                    ...
                    xnData.set(
                      {
                        yuangong_id: res.data.yuangong_id,
                        goodtype_id: vm.order.goodtype_id,
...
<<<<<<< HEAD
      vm.confirmOrder = function(id) {
        FirstService.confirmOrder(id).then(
          function(res) {
            if (res.status == 200) {
              ionicToast.alert('恭喜你收到我们的服务')
              $ionicHistory.goBack()
...
>>>>>>> 8b0c5b038174469ec9a596b16a6d9df57b4801f7
      ...
      vm.goPinkUp = function() {
        ...
        obj.subject = '家政服务超市商品'
        obj.rsa_private =
          ''
        obj.orderRetInfo = []
        var info = {}
        info.is_cmt = '0'
        info.order_id = vm.order.order_id
  ...
  .controller('setUp_ctrl', [
    ...
    function(
      ...
      vm.getvisiton = function() {
        var targetPath =
          'file:///storage/emulated/0/download/' + 'houseSupermarket.apk'
        var appType = '',
          type = '',
          vistion = ENV.version
        ...
        FirstService.getVistion({ version_num: vistion, type: type })
          .then(function(res) {
...
<<<<<<< HEAD
      vm.setDefultAddress = function(id) {
        FirstService.setDefultAddress(id).then(
          function(res) {
            if (res.status == 200) {
              // vm.dele.isLoading = true;
              // vm.init();
      ...
      vm.deleAddress = function(id) {
        FirstService.deleAddress(id).then(
          function(res) {
            if (res.status == 200) {
              vm.dele.isLoading = true
              vm.init()
...
>>>>>>> 8b0c5b038174469ec9a596b16a6d9df57b4801f7
  ...
  .controller('edit_address_list_ctrl', [
    '$log',
    '$scope',
    '$state',
    'Storage',
...
<<<<<<< HEAD
      ...
      vm.submitAppraise = function() {
        ...
        FirstService.addAppraise(vm.query).then(
          function(res) {
            if (res.status == 200) {
  ...
  .controller('feedback_list_ctrl', [
    ...
    function(
      ...
      vm.init = function(page) {
        if (!run) {
          run = true
          FirstService.getfeedbacklist(page)
            .then(
              function(result) {
                run = false
                $log.debug('获取反馈列表', result)
...
<<<<<<< HEAD
        FirstService.setfeedback(vm.query).then(
          function(result) {
            if (result.status == '200') {
              $ionicHistory.goBack()
            } else {
