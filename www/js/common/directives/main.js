...
angular
    ...
    return {
      ...
      controller: function(
        ...
        $scope.selectCity = function(item) {
          if (
            !$scope.data.options.limit ||
            $scope.data.options.limit.indexOf('ares') == -1
          ) {
            $scope.current = 'ares'
          }
  ...
  .directive('cityPicker', [
    ...
    '$log',
    'RscAlert',
    function($ionicModal, $timeout, areas, $log, RscAlert) {
      ...
      return {
        ...
        link: function(scope, element, attrs) {
          ...
          if (
            angular.isDefined(so.defaultAreaData) &&
            so.defaultAreaData.length > 1
          ) {
            vm.defaultAreaData = so.defaultAreaData
            vm.areaData = vm.defaultAreaData.join(vm.tag)
          } else {
            vm.defaultAreaData = ['北京', '东城区']
            vm.areaData = angular.isDefined(so.areaData)
              ? so.areaData.join(vm.tag)
              : '请选择城市'
          }
                ...
                if (vm.city) {
                  ...
                  vm.city
                    ? (vm.areaData =
                        ...
                        vm.tag +
                        vm.city.name +
                        (vm.area ? vm.tag + vm.area.name : ''))
                    : (vm.areaData = vm.province.name + vm.tag + vm.city.name)
                  $log.debug(
                    '所选区域信息',
                    vm.AreaService.areas[vm.city.CityID]
                  )
                } else {
                  vm.areaData = '请选择'
                  RscAlert.alert('请选择市')
          ...
          vm.initAreaData = function(AreaData) {
            ...
            if (AreaData[1] && vm.province) {
              // 初始化市
              console.log(
                'vm.AreaService.citys',
                vm.AreaService.citys[vm.province.ProID]
              )
            ...
            if (AreaData[2] && vm.city && vm.city.sub) {
              // 初始化区
            }
          }
          if (vm.watchChange) {
          ...
          element.on('click', function() {
            ...
            $ionicModal
              .fromTemplateUrl(
                'js/common/directives/templates/city_select.html',
                {
                  scope: scope,
