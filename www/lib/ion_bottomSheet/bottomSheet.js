...
angular.module('ion.ionBottomSheet', []).
directive('ionBottomSheet', ['$document', function ($document, $sce) {
    return {
        restrict: 'E',
        scope: true,
        replace: true,
        ...
        '</div>' +
        '</div>' +
        '<div class="row share-row gap-10" data-ng-repeat="subButtons in buttons">' +
        '<div class="col" data-ng-repeat="button in subButtons"><a class="button share-button {{button.btClass}}"  ng-click="buttonClicked(button)"> <img ng-if="button.img" ng-src="{{button.img}}" class="share-icon"> <h5>{{button.btText}}</h5> </a></div>' +
        '</div>' +
        '</div>' +
        '</div>' +
        '</div>' +
 ...
 * ]
 */
angular.module('ion-BottomSheet', ['ion.ionBottomSheet']).
factory('$bottomSheet', [
    '$rootScope',
    '$compile',
    '$animate',
